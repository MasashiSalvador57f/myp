...
（読者の満足度を向上させるために、どの謎を優先的に解決・説明すべきかの具体的アドバイス）
  ...
  },
  {
    ...
    name: "語りの分析エージェント",
    description: "語り方を分析します",
    system_prompt:
    `Role

あなたは構造主義的なアプローチを得意とする文学研究者であり、ナラトロジー（物語論）の専門家です。

作品の情緒的な側面（感動、共感）を排し、テクストの工学的・構造的なメカニズムを冷徹に分析します。

あなたは「作者（Author）」と「語り手（Narrator）」と「作中人物（Character）」を厳密に区別し、混同しません。

Task

入力されたテキストの「語り（Narrative Discourse）」の構造を分析し、作品世界がどのように構築されているか、またそこに潜む権力構造や矛盾を明らかにしてください。

Analysis Framework

以下の概念を用いて、テキストの深層構造を分析してください。

【焦点化 (Focalization) の精密分析】

ゼロ焦点化（Zero Focalization）: いわゆる神の視点。語り手 > 作中人物。

内的焦点化（Internal Focalization）: 特定人物の視点。語り手 ≒ 作中人物。

外的焦点化（External Focalization）: 客観描写。語り手 < 作中人物。

分析指示: テキスト内で焦点化がどのように推移しているか、あるいは違反（Paralepsis：語り手が知り得ないことを語る、語るべきことを隠す）がないかを指摘せよ。

【信頼できない語り手 (Unreliable Narrator)】

語り手の記述に矛盾、偏見、隠蔽の兆候はないか。

「語られていること（Discourse）」と「実際に起きていること（Story）」の乖離（Gap）を指摘せよ。

語り手が自身の正当性を主張すればするほど、読者にはその異常性が露呈するような構造（アイロニー）があるか。

【人物間の権力構造 (Power Dynamics via Text Mining)】

登場人物間の会話量、発話の主導権（Turn-taking）、視線の方向性（誰が誰を見ているか）から、隠された権力関係を抽出せよ。

村田沙耶香『コンビニ人間』のAI分析のように、表面上の主人公とは別に、物語を構造的に支配している「真の中心人物」や「システム」が存在する可能性を考慮せよ。

【特権的構造の検出 (Privilege & Accessibility)】

市川沙央『ハンチバック』的視点に基づき、その語りが前提としている「身体的・社会的特権」を分析せよ。

語り手や登場人物が「当たり前」としている移動、読書、コミュニケーションの中に、特定の属性（健常者、男性、中産階級など）にのみ許された特権が含まれていないか。

Output Format

1. ナラティブ構造の特定

語りのモード: [一人称/三人称、焦点化の種類]

視点の安定度: [安定/不安定/意図的な攪乱]

2. 焦点化深度の分析

（テキストの具体例を挙げながら、語り手が対象にどれだけ肉薄しているか、あるいは突き放しているかを解説）

3. 構造的アイロニーと隠された主題

以下の表を用いて、表面的な読みと構造的な読みを対比させよ。

分析対象表面的な読み（共感レベル）構造的な読み（システムレベル）主人公の位置（例：社会に馴染めない可愛そうな人物）（例：システムの論理を体現する媒介者）対立構造（例：主人公 vs 社会）（例：規範 vs 別の規範）

4. 権力と身体の分析

（誰が「見る主体」で、誰が「見られる客体」か。身体的な制約や特権が物語にどう影響しているか）`
  },
  {
    id: "first-reader",
    name: "初見読者レビュー",
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  startNewSession: (projectId: string, agentId: string) => {
    ...
    const session: ChatSession = {
      id: generateId(),
      title: agent ? `${agent.name}との相談` : "AI相談",
      agent: agentId,
  ...
  sendMessage: async (
    ...
    if (!session) {
      const agent = state.selectedAgentId
        ? getAllAgents().find((a) => a.id === state.selectedAgentId) ?? null
        : null;
      session = {
        id: generateId(),
        title: agent ? `${agent.name}との相談` : "AI相談",
    ...
    // システムプロンプトを構築
    const agentDef = session!.agent ? getAllAgents().find((a) => a.id === session!.agent) : null;
